...
void GUIDropDown::SetDropDownHeight(int height){
	if(height <= 0)height = (posY2-posY1)*6;
...
TTF_Font* GUIText::GetFont(int size){
	if((fontsSize > 0) && (fonts != NULL)){
		...
		else if((size-(int)fontsMinSize >= 0) && (size-fontsMinSize < fontsSize))return fonts[size-fontsMinSize];
		...
		else if((size-(int)GUITextDefault.fontsMinSize >= 0) && (size-GUITextDefault.fontsMinSize < GUITextDefault.fontsSize))return GUITextDefault.fonts[size-GUITextDefault.fontsMinSize];
	}
	return NULL;
}
...
void GUIElementRemove(void *element, GUIElementType eType){
	...
	for(size_t n=0; n<GUIElementList_size; n++){
		...
		if((GUIElementList[n].element==element)&&(GUIElementList[n].type==eType))rev = true;
	}
