...
void SDLInit(){
	...
	#endif
	SDLResolutionX = SDL_GetVideoInfo()->current_w;
	SDLResolutionY = SDL_GetVideoInfo()->current_h;
}
...
bool SDLSetVideo(int width, int height, bool resizable, bool fullscreen, bool noframe, bool useOpenGL){
	...
	Uint8 bpp = SDL_GetVideoInfo()->vfmt->BitsPerPixel; if(bpp>32||(bpp!=32&&bpp!=24&&bpp!=16&&bpp!=8))bpp=32;
	Uint32 flags = SDL_ANYFORMAT|SDL_DOUBLEBUF|((SDL_GetVideoInfo()->hw_available)?SDL_HWSURFACE:SDL_SWSURFACE)|((resizable)?SDL_RESIZABLE:0)|((fullscreen)?SDL_FULLSCREEN:0)|((noframe||fullscreen)?SDL_NOFRAME:0)|((useOpenGL)?SDL_OPENGL:0);
...
SDL_Surface* SDLNew(Uint16 width, Uint16 height, bool alpha, bool hardware, bool fix){
	...
	Uint8 bpp = SDL_GetVideoInfo()->vfmt->BitsPerPixel; if(bpp>32||(bpp!=32&&bpp!=24&&bpp!=16&&bpp!=8))bpp=32;
		...
		tmp = SDL_CreateRGBSurface((SDL_GetVideoInfo()->hw_available && hardware)?SDL_HWSURFACE:SDL_SWSURFACE,width,height,bpp,0xff000000,0x00ff0000,0x0000ff00,(alpha)?0x000000ff:0);
	#else
		tmp = SDL_CreateRGBSurface((SDL_GetVideoInfo()->hw_available && hardware)?SDL_HWSURFACE:SDL_SWSURFACE,width,height,bpp,0x000000ff,0x0000ff00,0x00ff0000,(alpha)?0xff000000:0);
	#endif
